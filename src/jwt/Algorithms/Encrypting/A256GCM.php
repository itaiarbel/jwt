 ...
 */
class A256GCM implements EncryptingAlgorithm
{
    const ENC = 'A192GCM';    
    ...
    {    
        return openssl_encrypt($data, 'aes-256-gcm', $key, $options=0, $iv, $tag);
    }

    static function decrypt($data, $key, $iv, &$tag)
    {
        return openssl_decrypt($data, 'aes-256-gcm', $key, $options=0, $iv, $tag);         
    }
